class Hint
	...
	def self.getHint(question, result)
		...
		# Provide a more detailed Hint
			...
			@hint = "Work from left to right, multiply the first number by the second number and add the result to the third number"
		elsif ((question =~ /^(\d\s\*\s\d\s\-\s\d)+$/)&& result == false)
			@hint = "Work from left to right, multiply the first number by the second number and subtract the third number from the result"
		elsif ((question =~ /^(\d\s\/\s\d\s\*\s\d)+$/)&& result == false)
			@hint = "Work from left to right, Divide the first number by the second number and multiply the result by the third number"
		elsif ((question =~ /^(\d\s\/\s\d\s\+\s\d)+$/)&& result == false)
